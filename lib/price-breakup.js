 ...
 *   Gold + Diamond + Making Charges + GST = Total
 *
 * Gold = net gold weight × karat rate (24K rate × karat purity).
 * GST = tax% of (Gold + Diamond + Making) — exclusive, then added.
...
export function buildPriceBreakup(product = {}) {
  ...
  const taxableBase = goldValue + diamondCost + makingCharge;
  const hasComponentBreakup = taxableBase > 0;

  // BlueStone: GST on (gold + diamond + making), then total = sum + GST
  ...
  if (diamondCost > 0 || diamondCarat || diamondCount) {
    const diamondDetail = [
  ...
  // Absorb any gap so displayed lines still explain the selling price.
