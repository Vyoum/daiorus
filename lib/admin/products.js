...
import { getProductSpecLines } from '../product-specs';
...
async function loadAdminProducts({ take = 100 } = {}) {
  ...
  return {
    ...
    products: products.map((product) => {
      ...
      const specSummary = getProductSpecLines(product)
        .map((line) => line.value)
        .slice(0, 3)
        .join(' · ');
      return {
...
function parseOptionalMm(value, label) {
  if (value === '' || value == null) return null;
  const parsed = Number(value);
...
function parseOptionalCarat(value) {
  if (value === '' || value == null) return null;
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed <= 0) return null;
...
function parseProductSpecFields(input) {
  return {
    diamondCount: parseOptionalDiamondCount(input.diamondCount),
    diamondCarat: parseOptionalCarat(input.diamondCarat),
    diamondCostInr: parseOptionalNonNegativeInrNullable(
      input.diamondCostInr,
      'Diamond cost',
    ),
    diamondQuality: String(input.diamondQuality || '').trim() || null,
    diamondType: String(input.diamondType || '').trim() || null,
    stoneCount: parseOptionalDiamondCount(input.stoneCount),
    stoneCarat: parseOptionalCarat(input.stoneCarat),
    stoneCostInr: parseOptionalNonNegativeInrNullable(
      input.stoneCostInr,
      'Stone cost',
    ),
    heightMm: parseOptionalMm(input.heightMm, 'Height'),
    widthMm: parseOptionalMm(input.widthMm, 'Width'),
    lengthMm: parseOptionalMm(input.lengthMm, 'Length'),
    metalColor: String(input.metalColor || '').trim() || null,
  };
}

function parseOptionalNonNegativeInrNullable(value, label) {
  if (value === '' || value == null) return null;
  return parseOptionalNonNegativeInr(value, label);
}
...
async function resolveProductPricing(input, parsed) {
  ...
  const { specs, makingChargeInr, taxPct, discountInr } = parsed;

  let rate24kPerGram = null;
  if (goldWeightGrams) {
    if (!material || !parseGoldKarat(material)) {
      throw new Error(
        'Select a karat (e.g. 14K, 18K) when entering net gold weight. Price breakup uses the karat rate, not the 24K rate.',
      );
    }
    const settings = await getGoldPricingSettings();
    rate24kPerGram = settings.rate24kPerGram || null;
  }

  let fixedNonGoldPriceInr = null;
  ...
  if (goldPricingEnabled) {
    ...
    if (shouldRebase) {
      const inferPrice = computeProductPriceInr({
        priceInr: enteredPriceInr,
        material,
        goldWeightGrams,
        goldPricingEnabled,
        goldRate24kAtPricingInr: rate24kPerGram,
        currentRate24kPerGram: rate24kPerGram,
        makingChargeInr,
        taxPct,
        discountInr,
        diamondCostInr: specs.diamondCostInr,
        stoneCostInr: specs.stoneCostInr,
      });

      const inferred = inferFixedNonGoldPrice({
        sellingPriceInr: inferPrice > 0 ? inferPrice : enteredPriceInr,
        goldWeightGrams,
        material,
        rate24kPerGram,
      });
  ...
  const priceInr = computeProductPriceInr({
    priceInr: enteredPriceInr,
    material,
    goldWeightGrams,
    goldPricingEnabled,
  ...
  return {
    priceInr: priceInr > 0 ? priceInr : enteredPriceInr,
    goldPricingEnabled,
    goldWeightGrams,
    fixedNonGoldPriceInr: goldPricingEnabled ? fixedNonGoldPriceInr : null,
    goldRate24kAtPricingInr: rate24kPerGram,
  };
}
...
export async function createAdminProduct(input) {
  ...
  const discountInrRaw = parseOptionalNonNegativeInr(input.discountInr, 'Discount');
  const discountInr = discountInrRaw && discountInrRaw > 0 ? discountInrRaw : null;
  const material = String(input.material || '').trim() || null;
  const specs = parseProductSpecFields(input);
  ...
  return prisma.product.create({
    data: {
      ...
      makingChargeInr,
      taxPct,
      diamondCount: specs.diamondCount,
      diamondCarat: specs.diamondCarat,
      diamondCostInr: specs.diamondCostInr,
      diamondQuality: specs.diamondQuality,
      diamondType: specs.diamondType,
      stoneCount: specs.stoneCount,
      stoneCarat: specs.stoneCarat,
      stoneCostInr: specs.stoneCostInr,
      heightMm: specs.heightMm,
      widthMm: specs.widthMm,
      lengthMm: specs.lengthMm,
      metalColor: specs.metalColor,
      productInfo,
      tag,
