...
export async function getMediaLibraryContent() {
  const [announceRow, heroRow, signatureRow, curatedSelectsRow] = await Promise.all([
...
export async function saveMediaLibraryContent({ announce, hero, signature, curatedSelects }) {
  const ops = [];
  ...
  if (curatedSelects) {
    const row = curatedSelectsToRow(curatedSelects);
    ops.push(
      prisma.siteContent.upsert({
        where: { key: row.key },
        create: row,
