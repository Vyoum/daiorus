...
export function calculateCartTotals(items, { discountInr = 0, shippingInr: shippingOverride } = {}) {
  ...
  const shippingInr =
    typeof shippingOverride === 'number' ? shippingOverride : SHIPPING_FEE_INR;
  const maxDiscount = subtotalInr + shippingInr;
  const safeDiscount = Math.min(Math.max(0, Math.round(discountInr || 0)), maxDiscount);
