...
export function normalizeProductSpecs(source = {}) {
  ...
  return {
    ...
    diamondCount,
    diamondCarat,
    diamondQuality: String(source.diamondQuality || '').trim() || null,
    diamondType: String(source.diamondType || '').trim() || null,
    stoneCount,
    stoneCarat,
    heightMm:
    ...
    widthMm:
      source.widthMm == null || source.widthMm === ''
        ...
        : Number.isFinite(Number(source.widthMm))
          ...
          : null,
    lengthMm:
      source.lengthMm == null || source.lengthMm === ''
        ? null
        : Number.isFinite(Number(source.lengthMm))
          ? Number(source.lengthMm)
          : null,
...
export function getProductSpecLines(source = {}) {
  ...
  if (lengthMm) lines.push({ label: 'Length', value: `${lengthMm} mm` });
  ...
  if (diamondQuality) lines.push({ label: 'Diamond quality', value: diamondQuality });
  if (diamondType) lines.push({ label: 'Diamond type', value: diamondType });
