...
function buildCreateShipmentPayload(order, shipping, { testBooking = false } = {}) {
  const config = getSequelConfig();
  const boxNumber = `DAI-${order.orderNumber}`.replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 20);
  const remark = testBooking
    ? `Daiorus TEST order ${order.orderNumber} (payment pending)`
    : `Daiorus order ${order.orderNumber}`;
...
function resolveShippingForBooking(order, { allowPendingPayment = false } = {}) {
  const config = getSequelConfig();
  const base = getShippingFromOrder(order) || {};

  let customerName = String(base.fullName || '').trim();
  if (!customerName && order.user?.name) customerName = String(order.user.name).trim();
  if (!customerName && order.guestEmail) customerName = order.guestEmail.split('@')[0];

  const phone = normalizePhone(base.phone);
  const line1 = String(base.line1 || '').trim();
  const postalCode = String(base.postalCode || '').trim();

  if (line1 && postalCode && customerName && phone) {
    return {
      ...base,
      ...
      line1,
      postalCode,
      country: base.country || 'IN',
    };
  }

  if (!allowPendingPayment || order.status !== 'PENDING') {
    return null;
  }

  return {
    fullName: customerName || 'Test Customer',
    phone: phone || normalizePhone(process.env.SEQUEL247_TEST_PHONE) || '9999999999',
    line1: line1 || '411, DLF Prime Towers, Okhla Phase 1',
    line2: base.line2 || 'Delhi',
    city: base.city || 'Delhi',
    state: base.state || 'Delhi',
    postalCode: postalCode || config.originPincode || '110020',
    country: 'IN',
  };
}
...
export async function bookShipmentForOrder(orderId, { force = false, allowPendingPayment = false } = {}) {
  ...
  const shipping = resolveShippingForBooking(order, { allowPendingPayment });
  ...
  const testBooking = allowPendingPayment && order.status === 'PENDING';
  const payload = buildCreateShipmentPayload(order, shipping, { testBooking });
